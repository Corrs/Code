$(document).ready(function() {
	var addPageUrl = basePath + "/content/append",
		$table = $('#table'),
		$remove = $('#remove'),
		$append = $('#append'),
		$TABLEPANEL = $('#tablePanel'),
		catalogList = {"banner": "顶部图片", "carousel": "轮播图片", "sub": "附属图片"};
//		selections = [];
	
	initTable();
	jr.initFade();
	try{
		var result = $.ajax({url: basePath + "/sys/catalog/query", async:false});
		console.log(result);
		if(result && result.responseText) {
			var json = $.parseJSON(result.responseText);
			if(json.success) {
				for(var i=0; i<json.rows.length; i++) {
					catalogList[json.rows[i].code] = json.rows[i].name;
				}
			}
		}
	} catch (e) {
	...
	function initTable() {
		$table.bootstrapTable({
			...
			columns: [
				...
				},
				{
					...
					align: 'center',
					formatter: function(value, row, index) {
						return catalogList[value]?catalogList[value]:value;
					}
				},
				{
					field: "remark",
		...
		$table.on('load-success.bs.table', function() {
			...
			$('[modelid]').on('click', function(e){
				e.preventDefault();
				jr.loadPage(basePath+"/content/edit/" + $(this).attr('modelid'));
			});
			...
			$('[delete]').on('click', function(e) {
				...
				function callback(result) {
					if(result) {
						jr.operate($table, basePath+"/content/delete/"+id, null);
					}
			    }
			});
		});
	...
	$remove.on('click', function() {
		...
		function callback(result) {
			if(result) {
				jr.deleteDatas($table, basePath+"/content/delete");
			}
	    }
    });
});