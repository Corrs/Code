$(document).ready(function() {
	var addPageUrl = basePath + "/sys/content/append",
		$table = $('#table'),
		$remove = $('#remove'),
		$append = $('#append'),
...
//		selections = [];
	...
	function initTable() {
		$table.bootstrapTable({
			url: basePath+'/sys/content/query',
			method: 'post',
		...
		$table.on('load-success.bs.table', function() {
			...
			$('[modelid]').on('click', function(e){
				e.preventDefault();
				jr.loadPage(basePath+"/sys/content/edit/" + $(this).attr('modelid'));
			});
			...
			$('[delete]').on('click', function(e) {
				...
				function callback(result) {
					if(result) {
						jr.operate($table, basePath+"/sys/content/delete/"+id, null);
					}
			    }
			});
		});
...
//		debugger;
	...
	$remove.on('click', function() {
		...
		function callback(result) {
			if(result) {
				jr.deleteDatas($table, basePath+"/sys/content/delete");
			}
	    }
    });
