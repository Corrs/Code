$(document).ready(function() {
	var addPageUrl = basePath + "/sys/user/append",
		$table = $('#table'),
		$remove = $('#remove'),
		$append = $('#append'),
...
//		selections = [];
	...
	function initTable() {
		...
		$table.on('load-success.bs.table', function() {
			...
			$('[modelid]').on('click', function(e){
				e.preventDefault();
				jr.loadPage(basePath+"/sys/user/edit/" + $(this).attr('modelid'));
			});
			...
			$('[delete]').on('click', function(e) {
				...
				function callback(result) {
					if(result) {
						jr.operate($table, basePath+"/sys/user/delete/"+id, null);
					}
			    }
			});
		});
	...
	$remove.on('click', function() {
		...
		function callback(result) {
			if(result) {
				jr.deleteDatas($table, basePath+"/sys/user/delete");
			}
	    }
    });
});