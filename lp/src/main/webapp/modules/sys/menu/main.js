$(document).ready(function() {
	var addPageUrl = basePath + "/sys/menu/append",
		$table = $('#table'),
		$remove = $('#remove'),
		$append = $('#append'),
		$checkableTree = $('#treeview-checkable'),
		$TREEPANEL = $('#treePanel'),
...
//		selections = [];
	...
	// 树缩放，并使列表自适应
	function initTreeFade() {
		$("#fade-link").on("click", function(e) {
			e.preventDefault();
			var _this = $(this);
			$TREEPANEL.fadeToggle();
			if ($(this).find('i').is('.fa-times')) {
				_this.html('<i class="fa fa-bars fa-fw"></i>');
				$TABLEPANEL.removeClass("col-md-10").addClass("col-md-12");
			} else {
				_this.html('<i class="fa fa-times fa-fw"></i>');
	...
	function initTree() {
		...
		$.post(basePath + "/sys/menu/queryTree", function(data, textStatus, jqXHR){
			var result = $.parseJSON(data.replace(/"nodes":\[\]/g, '"nodes":null'));
			...
			if(result.success) {
				...
				$checkableTree.on('nodeSelected nodeUnselected', function(event, node){
					$append.prop('disabled', !$checkableTree.treeview('getSelected').length);
					// 点击树刷新列表数据
					var options = {};
					if ($checkableTree.treeview('getSelected').length) {
						options.queryParams = {
							pcode: $checkableTree.treeview('getSelected')[0].code,
							order: 'asc',
							sort: 'code'
						};
					}
				...
				init_PNotify("错误", "树组件初始化错误", "error", 3000);
			}
		});
	}
	...
	function initTable() {
		...
		$table.on('load-success.bs.table', function() {
			...
			$('[modelid]').on('click', function(e){
				e.preventDefault();
				jr.loadPage(basePath+"/sys/menu/edit/" + $(this).attr('modelid'));
			});
			...
			$('[delete]').on('click', function(e) {
				...
				function callback(result) {
					if(result) {
						jr.operate($table, basePath+"/sys/menu/delete/"+id, null, refreshTree);
					}
			    }
			});
		});
...
//		debugger;
//		($checkableTree.treeview('getSelected').length)?$checkableTree.treeview('getSelected')[0].code:null;
		var code = $checkableTree.treeview('getSelected')[0].code;
		jr.loadPage(addPageUrl+"/"+(code?code:null));
    });
	...
	$remove.on('click', function() {
		...
		function callback(result) {
			if(result) {
				jr.deleteDatas($table, basePath+"/sys/menu/delete", refreshTree);
			}
	    }
    });
