       ...
       <div class="x_panel">
         ...
         <div class="x_content jrwl_custom_content" >
         	<form class="form-horizontal form-label-left" id="saveForm" method="post" action="<c:out value="${sessionScope.basePath }"></c:out>/form/save">
         	   ...
         	   <input type="hidden" name="sn" value="<c:out value="${model.sn }"></c:out>">
         	   <input type="hidden" name="createdate" value="<c:out value="${model.createdate }"></c:out>">
         	   <input type="hidden" name="oldFieldName" value="<c:out value="${model.fieldname }"></c:out>"> 
               <div class="item form-group">
                 <label class="control-label col-md-3 col-sm-3 col-xs-12" for="fieldname">字段名称 <i class="required text-danger">*</i>
                 </label>
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   	<input class="form-control col-md-7 col-xs-12" id="fieldname" type="text" name="fieldname" value="<c:out value="${model.fieldname }"></c:out>" placeholder="字段名称必须为英文字符，内容长度不超过20">
                 </div>
               </div>
               <div class="item form-group">
                 ...
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   	<select class="form-control col-md-7 col-xs-12" name="fieldtype" >
                   	 <option value="1" <c:if test="${model.fieldtype eq 1 }">selected=""</c:if>>字符串</option>
                     <option value="4" <c:if test="${model.fieldtype eq 4 }">selected=""</c:if>>文本域</option>
                     <option value="2" <c:if test="${model.fieldtype eq 2 }">selected=""</c:if>>数字</option>
                     <%-- <option value="3" <c:if test="${model.fieldtype eq 3 }">selected=""</c:if>>时间</option> --%>
                   </select>
                 </div>
               </div>
               <div class="item form-group">
                 ...
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   	<input class="form-control col-md-7 col-xs-12" id="label" type="text" name="label" value="<c:out value="${model.label }"></c:out>" placeholder="请输入标签名，内容长度不超过20">
                 </div>
               </div>
               <div class="item form-group">
                 ...
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   	<input class="form-control col-md-7 col-xs-12" id="placeholder" type="text" name="placeholder" value="<c:out value="${model.placeholder }"></c:out>" placeholder="请输入提示语，内容长度不超过50">
                 </div>
               </div>
