       ...
       <div class="x_panel">
         ...
         <div class="x_content jrwl_custom_content" >
         	<form class="dropzone" dropzone="" id="dropzone">
                ...
                <input type="hidden" name="catalog" value="<c:out value="${model.catalog }"></c:out>">
                <input type="hidden"  value="<c:out value="${model.remark }"></c:out>" name="remark">
         	...
         	<form class="form-horizontal form-label-left" id="saveForm" method="post" action="<c:out value="${sessionScope.basePath }"></c:out>/content/save">
         	   <input type="hidden" name="id" value="<c:out value="${model.id }"></c:out>">
         	   <input type="hidden" name="image" value="<c:out value="${model.image }"></c:out>">
         	   <input type="hidden" name="createdate" value="<c:out value="${model.createdate }"></c:out>">
               <div class="item form-group">
                 ...
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   <select class="form-control col-md-7 col-xs-12" name="catalog" >
                   	 <option <c:if test="${model.catalog eq 'banner' }">selected=""</c:if> value="banner">顶部图片</option>
                     <option <c:if test="${model.catalog eq 'carousel' }">selected=""</c:if> value="carousel">轮播图片</option>
                     <option <c:if test="${model.catalog eq 'sub' }">selected=""</c:if> value="sub">附属图片</option>
                     <c:forEach items="${catalogList }" var="catalog">
                     <option <c:if test="${model.catalog eq catalog.code }">selected=""</c:if> value="<c:out value="${catalog.code }"></c:out>"><c:out value="${catalog.name }"></c:out></option>
                     </c:forEach>
                   </select>
                 </div>
               </div>
               <div class="item form-group">
                 ...
                 <div class="col-md-6 col-sm-6 col-xs-12">
                   	<input class="form-control col-md-7 col-xs-12" id="remark" type="text"  value="<c:out value="${model.remark }"></c:out>" name="remark">
                 </div>
               </div>
